...
nBOA <- function(y, experts, awake = NULL, loss.type = "square", loss.gradient = TRUE, 
                w0 = NULL, training = NULL) {
  
  experts <- as.matrix(experts)
  nexperts <- cbind(experts, 0)
  ...
  nawake <- cbind(awake,1)
  
  idx.na <- which(is.na(nexperts))
  nawake[idx.na] <- 0
  nexperts[idx.na] <- 0
  
  R <- rep(0, N+1)
  R.reg <- R
  weights <- matrix(0, ncol = N+1, nrow = T)
  prediction <- rep(0, T)
  w <- w0
  eta <- matrix(exp(350), ncol = N+1, nrow = T + 1)
  V <- 0
  B <- 0
